# Plate-Motion Wake Animation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: superpowers:subagent-driven-development. Steps use checkbox (`- [ ]`) syntax.

**Goal:** Replace the dashed-line flow on plate boundaries with directional wake animations — soft elliptical bands that emerge near the line and propagate in the correct motion direction (outward at ridges, inward at subduction, along-line at transforms), with cycle period encoding magnitude.

**Architecture:** New module `src/plateWake.js` builds a per-side offset strip mesh for each boundary group plus a shared `ShaderMaterial` whose fragment shader phases the wake by mode + per-vertex attributes. Wires into `plateMotion.js`'s update loop. The atlas's existing `LineMaterial`s stay (lines render solid + colored), with `dashed: false` permanently — the wake replaces the dash animation.

**Spec:** `docs/superpowers/specs/2026-04-28-plate-motion-wake-design.md`

**Branch:** `feat/plate-motion`

**Locked settings (constants in plateWake.js):**
```js
const WAKE = {
    travelKm:      150,    // strip extends 150 km perpendicular to line per side
    bandThickness: 1.5,    // visible band thickness in pixels (relative cue)
    startW:        1.0,
    endW:          2.0,
    cycleFastSec:  3.0,    // for ~150 mm/yr boundary
    cycleSlowSec:  12.0,   // for ~3 mm/yr boundary
    concurrent:    3,
    peakOpacity:   0.20,
    fadeIn:        0.08,
};
...
`travelKm = 150`: tuner slider was 38 px on a 400 px viewbox. Translating 38 px to world km on a 6367 km globe rendered at typical zoom comes out to ~150 km on the surface. Tunable later.
...
```
NEW
  src/plateWake.js                   # all wake geometry + shader + lifecycle
  scripts/inspect-plate-wake.js      # Playwright capture
MODIFIED
  src/plateMotion.js                 # remove dashOffset animation; build + update plateWake
  src/atlas.js                       # leave dashed: false permanently (already that way)
  src/main.js                        # nothing (plateWake is owned by plateMotion)
```

---

## Task 1: Scaffold `src/plateWake.js` with strip geometry

**Files:** Create `src/plateWake.js`

The module exports `loadPlateWake({ scene, atlas, poleIndex, classMode })` returning `{ update(t), dispose() }`. It builds, for each boundary group, two `Mesh`es (one per side of the line) carrying the wake `ShaderMaterial`. Each mesh's geometry is a strip — quads — generated from the group's boundary segments, offset perpendicular to each segment by `travelKm`.

- [ ] **Step 1: write the imports + constants**
...
Helper: given a group's `features` (each is a 2-point LineString), build a single combined `BufferGeometry` containing strip quads for one side of the line. Returns the geometry plus the side parameter (-1 or +1).
...
- [ ] **Step 4: build the public function**
...
```bash
git add src/plateWake.js
git commit -m "Add plateWake: directional wake animation strips per boundary group

Strip mesh on each side of every boundary line; fragment shader
phases a soft band that propagates outward (divergent), inward
(convergent), or along-line (transform), based on a mode uniform.
Cycle period interpolates between 12 s (slow) and 3 s (fast) by the
group's avg sqrt-compressed velocity magnitude.
...
## Task 2: Wire `plateWake` into `plateMotion.js`

**Files:** Modify `src/plateMotion.js`

The existing flow-via-dashOffset code goes away; the wake takes over the motion-animation role.
...
- [ ] **Step 2: instantiate it after arrow setup**

In `loadPlateMotion`, AFTER `populateArrowInstances(arrowMesh, samples, radius);`, add:

```js
    const plateWake = loadPlateWake({ scene, atlas, poleIndex });
```

- [ ] **Step 3: drop the old groupFlowSpeeds + dashOffset animation**

Find the `groupFlowSpeeds` const + the `update(t)` body that sets `mat.dashOffset`. Replace with:
...
(The wake is now the only thing animating per frame; arrow instance matrices are static.)

- [ ] **Step 4: smoke-test by reload**

The dev server should already be running. Open `http://localhost:3000/`. The boundary lines should now have soft glows on each side that pulse outward (ridges), inward (subduction), or along-line (transforms).

- [ ] **Step 5: commit**

```bash
git add src/plateMotion.js
git commit -m "plateMotion: replace dashOffset flow with plateWake

The dashed-line drift was directionally correct only on transforms —
on ridges/subduction the motion is perpendicular, not along the line.
plateWake renders directional wake strips that propagate in the
correct direction per boundary class. Removes the groupFlowSpeeds
plumbing and the dashOffset animation."
```
...
**Files:** Create `scripts/inspect-plate-wake.js`
...
```bash
bun run scripts/inspect-plate-wake.js
```

Open `/tmp/eqdev/wake-t0.png`, `wake-t2.png`, `wake-t4.png`. Pixel-diff (or visual compare) the three: wakes should have moved between frames. Boundary lines visibly soft-glowy on either side.

- [ ] **Step 3: commit**

```bash
git add scripts/inspect-plate-wake.js
git commit -m "scripts: inspect-plate-wake captures three frames 2 s apart"
```
...
Spec coverage:
- ✅ Per-side wake strips with mode-driven direction → Task 1
- ✅ Sqrt-compressed magnitude → cycle period mapping → Task 1 Step 4
- ✅ Three concurrent staggered wakes → encoded in shader's `fract(phase * concurrent)` math (handled in the shader)
- ✅ Settings constants from the locked tuner JSON → Task 1 Step 1
- ✅ Replace existing dashOffset animation → Task 2
- ✅ Visual smoke test → Task 3

No placeholders. No TBDs. Type names consistent: `loadPlateWake`, `WAKE` constant, `TYPE_MODE` map.
